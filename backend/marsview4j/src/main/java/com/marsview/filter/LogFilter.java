...
//        }
    ...
    @Override
    public Object callableStatement_getObject(FilterChain chain, CallableStatementProxy statement, int parameterIndex, java.util.Map<String, Class<?>> map) throws SQLException {
    ...
    @Override
    public Object callableStatement_getObject(FilterChain chain, CallableStatementProxy statement, String parameterName, java.util.Map<String, Class<?>> map) throws SQLException {
...
//            statementLog("{conn-" + statement.getConnectionProxy().getId() + ", stmt-" + statement.getId() + "} created");
    ...
    private void showSql(StatementProxy statement, String sql) {
        statement.setLastExecuteTimeNano();
        if (!StringUtils.equals("SELECT 1", sql)) {
            double nanos = statement.getLastExecuteTimeNano();
            double millis = nanos / (1000 * 1000);

            String schemaName = "";
            try {
                schemaName = statement.getConnection().getCatalog();
            ...
            statementLog("Actual SQL: " + schemaName + " ::: " + sql + " 执行耗时:" + new BigDecimal(millis / 1000).setScale(3, BigDecimal.ROUND_HALF_UP) + "秒");
        }
    }
}
