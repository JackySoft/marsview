 ...
 * @returns 返回组件
 */
const ColumnChart = ({ config }: ComponentType, ref: any) => {
    ...
    handleApi(config.api, data).then((res) => {
      if (res?.code === 0) {
        if (Array.isArray(res.data)) {
          ...
          console.error('[ColumnChart]', 'data数据格式错误，请检查');
          setData([]);
        }
      }
      setLoading(false);
    ...
    visible && (
      <div style={config.style}>
        <Spin spinning={loading} size="large" wrapperClassName="spin-loading">
          <Column {...config.props} color={config.props.seriesField ? config.props.color : config.props.color[0]} data={data} />
        </Spin>
      </div>
    )
  );
};
export default forwardRef(ColumnChart);
